...
- Explicit saved Codex model assignments are preserved on sync, except the legacy default carril set (`sdd-strong=gpt-5.5`, `sdd-mid=gpt-5.5`, `sdd-cheap=gpt-5.4-mini`), which is migrated to the current GPT-5.6 defaults. GPT-5.6 IDs are also used for missing/default assignments.
